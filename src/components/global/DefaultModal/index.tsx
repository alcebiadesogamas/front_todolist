import React, { memo, useCallback, useState, Fragment } from 'react';
import { Dialog, Transition } from '@headlessui/react';
import style from './defaultModal.module.css';
import { useModal } from '../../../hooks/useModal';
import { HomePageController } from '../../../pages/home/HomePageController';

export const DefaultModal: React.FC = memo(() => {
  const { handleModalClose, modal, handleModalInputChange } = useModal();

  return (
    <>
      <Transition appear show={modal.isOpen} as={Fragment}>
        <Dialog
          as="div"
          className="fixed inset-0 z-10 overflow-y-auto"
          onClose={() => null}
        >
          <div className="min-h-screen px-4 text-center">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0"
              enterTo="opacity-10"
              leave="ease-in duration-200"
              leaveFrom="opacity-10"
              leaveTo="opacity-0"
            >
              <Dialog.Overlay className="fixed inset-0 bg-black opacity-50" />
            </Transition.Child>

            <span
              className="inline-block h-screen align-middle"
              aria-hidden="true"
            >
              &#8203;
            </span>
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0 scale-95"
              enterTo="opacity-100 scale-100"
              leave="ease-in duration-200"
              leaveFrom="opacity-100 scale-100"
              leaveTo="opacity-0 scale-95"
            >
              <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                <Dialog.Title
                  as="h3"
                  className="text-lg font-medium leading-6 text-gray-900"
                >
                  Informe o nome
                </Dialog.Title>

                <div className="mt-2">
                  <input
                    onChange={(e) => handleModalInputChange(e.target.value)}
                    className={style.modalInput}
                  />
                </div>
                <div className={style.modalDiv3}>
                  <button
                    onClick={handleModalClose}
                    className={style.modalButtonCancel}
                  >
                    Cancelar
                  </button>
                  <button
                    onClick={() => modal.callback(modal.inputValue)}
                    disabled={!modal.inputValue}
                    className={style.modalButtonConfirm}
                  >
                    Confirmar
                  </button>
                </div>
              </div>
            </Transition.Child>
          </div>
        </Dialog>
      </Transition>
    </>
  );
});
